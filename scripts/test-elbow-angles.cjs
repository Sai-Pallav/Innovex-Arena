...
async function main() {
  ...
  // Setup isolated scene for rendering
  await page.evaluate(() => {
  ...
  async function renderAngle(angleRad, label) {
    ...
    // 1. Front view of full right arm
    ...
    // 3. 3/4 Front view of full right arm
  ...
  await renderAngle(-0.28, 'ang_028');
  await renderAngle(-0.38, 'ang_038');
  await renderAngle(-0.44, 'ang_044');
  await renderAngle(-0.50, 'ang_050');
