...
function createBearingHousing(
  ...
  // 1. Inboard stepped annular collar sleeve
...
/**
 * Creates the arm mounting flange (Callout 1) interfacing into UpperArm.ts.
 */
function createArmMountingFlange(
  ...
  flangeMesh.position.set(side * 0.018, 0, 0);
  ...
  rimGeo.rotateY(Math.PI / 2);
  const rim = new THREE.Mesh(rimGeo, materials.metallic);
  rim.position.set(side * (0.018 + flangeThickness * 0.5), 0, 0);
  flangeGroup.add(rim);
  ...
  for (let b = 0; b < boltCount; b++) {
    ...
    socket.position.set(
      side * (0.018 + flangeThickness * 0.48),
    ...
    boltHead.position.set(
      side * (0.018 + flangeThickness * 0.55),
      Math.sin(angle) * boltPitchR,
      Math.cos(angle) * boltPitchR
    );
  ...
  // Deep Hollow Central Bearing Bore extending inward
  const boreLen = 0.030;
  ...
  boreMesh.position.set(side * 0.005, 0, 0);
  ...
  innerRaceGeo.rotateZ(Math.PI / 2);
  const innerRace = new THREE.Mesh(innerRaceGeo, materials.metallic);
  innerRace.position.set(-side * 0.003, 0, 0);
...
export function createMultiAxisShoulderJoint(
  ...
  // Natural forward-lateral orientation: angles circular bearing face towards camera (+Z)
  shoulderJoint.rotation.set(0.02, -side * 0.20, side * 0.04);
  ...
  const { housingGroup, housingMesh, accentRing } = createBearingHousing(side, materials, ledMeshes);
  primaryAxisPivot.add(housingGroup);

  // Central Rotating Hub
  const { hubGroup, driveHub } = createPrimaryRotationHub(side, materials);
